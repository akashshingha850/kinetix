...
- **Drone-based Systems**: Commercial solutions (DJI Terra, Pix4D) use pre-planned missions with high overlap (70–90%). Mostegel [\[2\]](#ref2) proposes quality-driven autonomous re-capture for UAVs using multi-view-stereo confidence prediction, representing an early online feedback loop. Pan [\[3\]](#ref3) recently extends this to full on-the-fly SfM with predictive path planning.
...
- **Classic 2D Coverage (Boustrophedon / Lawnmower)**: Parallel back-and-forth sweeps over a polygon, implemented by DJI Terra, Pix4DCapture, and most commercial planners. Bähnemann [\[7\]](#ref7) reformulates this as a Generalized Traveling Salesman Problem to minimize transition costs between cells, achieving 14% lower path cost. These methods are efficient on flat terrain but are open-loop — they do not respond to reconstruction quality and generate far more images than necessary.
- **3D Coverage for Terrain & Structures**: Torres [\[8\]](#ref8) extends CPP to 3D terrain, generating UAV paths that minimize battery consumption while achieving photogrammetric coverage of complex surfaces. Jing [\[9\]](#ref9) addresses large structures (buildings) with multi-UAV CPP. These approaches require a known 3D model as input — they are model-based and cannot handle unknown or partially known indoor objects.
- **Adaptive CPP**: Peng and Isler [\[10\]](#ref10) propose an explore-then-exploit approach for aerial 3D reconstruction: an initial exploratory flight builds a coarse proxy, and subsequent exploitation flights target low-quality regions. Stache [\[11\]](#ref11) adapts UAV altitude and resolution online based on incoming semantic segmentation quality. These works demonstrate that adaptive replanning substantially reduces flight time and image count.
- **Survey**: Maboudi [\[12\]](#ref12) provides a comprehensive review of model-free and model-based viewpoint and path-planning algorithms for UAV-based 3D reconstruction, covering both CPP and NBV paradigms and documenting the shift toward adaptive, scene-aware methods.
...
- **Next Best View Planning**: Classic problem formalized by Connolly [\[13\]](#ref13) and surveyed comprehensively by Scott [\[14\]](#ref14).
- **Uncertainty-based NBV**: Methods using volumetric uncertainty or information gain, e.g., receding-horizon voxel exploration [\[15\]](#ref15), a foundational information-gain formulation [\[16\]](#ref16), and a rigorous comparison of information-gain metrics for object reconstruction [\[17\]](#ref17). Vasquez-Gomez [\[18\]](#ref18) extends these to object-level reconstruction with explicit positioning uncertainty.
- **Learning-based View Planning**: Mendoza and Vasquez-Gomez [\[19\]](#ref19) use a supervised 3D-CNN to predict the NBV from partial point clouds. Peralta [\[20\]](#ref20) applies reinforcement learning (Scan-RL) to learn a view policy for drone-based scanning. More recently, NeurAR [\[21\]](#ref21) and Neu-NBV [\[22\]](#ref22) integrate NBV selection with neural-rendering uncertainty estimation.
- **Frontier-Based Exploration**: Yamauchi [\[23\]](#ref23) introduces frontier-based exploration — navigating toward boundaries between known free space and unknown space — which has become a standard baseline for autonomous exploration planners including [\[15\]](#ref15).
...
- **Autonomous Exploration**: Bircher [\[15\]](#ref15) demonstrates environment-level exploration with UAVs using receding-horizon NBV. Schmid [\[31\]](#ref31) addresses autonomous volumetric exploration of large-scale environments under severe odometry drift.
- **Object-centric Scanning**: Kriegel [\[32\]](#ref32) proposes efficient next-best-scan planning for object surface reconstruction using robotic arms — principles applicable but not directly transferable to drone platforms.
...
- **Monocular-only active reconstruction indoors**: All confirmed active reconstruction systems with real-world validation use metric depth (RGB-D, LiDAR, or ToF) or stereo cameras. Bircher [\[15\]](#ref15), Delmerico [\[17\]](#ref17), NeurAR [\[21\]](#ref21), and SOAR [\[34\]](#ref34) all assume depth input. Kinetix's monocular-only constraint for indoor drones has no direct prior.
